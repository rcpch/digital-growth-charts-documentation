...
| **5.7 System testing** | The API is tested end-to-end. The **~4000-child static test harness** provides mathematically reproducible evidence of calculation accuracy for a selected reference; separate provenance and component tests verify reference identity across calculation, serialization and presentation (see below). |
...
- **Static test harness** — a reference dataset of approximately **4000 children** is used to verify that the API returns the expected centile/SDS values for the selected reference. Because the LMS calculation is deterministic, this harness provides **mathematically reproducible** evidence of arithmetic accuracy and detects any regression introduced by a code or dependency change. It does not prove that every layer selected or displayed the same reference.
- **Reference-integrity tests** - calculation-engine tests stamp all six canonical references; API tests verify pass-through identity, schema constraints, bulk output and fictional-child output; component tests exercise matching, legacy, unknown, mismatched and mixed provenance in centile and SDS charts. Together these tests address the end-to-end presentation consistency that the static harness alone cannot establish. The released contract and evidence links are recorded in the [Growth reference provenance contract](https://github.com/rcpch/digital-growth-charts-documentation/blob/live/spec/growth-reference-provenance-contract.md).
